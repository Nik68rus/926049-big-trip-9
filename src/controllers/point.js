...
export default class PointController {
  constructor(container, trip, eventInfo, mode, onDataChange, onChangeView, destinations, offers) {
  ...
  init(mode) {
    ...
    const onEscKeyDown = (evt) => {
      if (isEscapeKey(evt)) {
        if (mode === Mode.ADDING) {
          eventEditElement.remove();
          addBtn.disabled = false;
          this._trip.renderEmptyMessage();
        } else {
          this._eventEdit.resetForm();
          eventEditElement.replaceWith(eventViewElement);
        }
        document.removeEventListener(`keydown`, onEscKeyDown);
      }
    };
    ...
    priceInput.addEventListener(`keyup`, onPriceFill);
  }
